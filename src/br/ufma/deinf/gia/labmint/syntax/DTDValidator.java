...
public class DTDValidator {
	...
	public static boolean validate(String fileName, Element root) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (nclStructure.isChild(tagName, childTagName)) {
					if (childSet.containsKey(childTagName)) {
						childSet.put(childTagName,
								childSet.get(childTagName) + 1);
					} else
						childSet.put(childTagName, 1);
				} else {
					Vector<String> args = new Vector<String>();
					args.add(childTagName);
					args.add(tagName);
					MessageList.addError(fileName, 2005, child, args);
					ret = false;
				}
			}
		}
		...
		Map<String, Character> childCard = nclStructure
		...
		while (it.hasNext()) {
			...
			case 'a':
				if (childSet.containsKey(entry.getKey())) {
					if (childSet.get(entry.getKey()) == 1 && !flagAtLeastA)
						flagAtLeastA = true;
					else
						flagAtLeastA = false;
				}
			...
			case 'b':
				if (childSet.containsKey(entry.getKey())) {
					if (childSet.get(entry.getKey()) == 1 && !flagAtLeastB)
						flagAtLeastB = true;
					else
						flagAtLeastB = false;
				}
		...
		for (int i = 0; i < childList.getLength(); i++) {
			...
			if (child.getNodeType() == Node.ELEMENT_NODE) {
				if (!validate(fileName, (Element) child))
					ret = false;
			}
		}
		return ret;
