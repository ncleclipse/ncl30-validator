...
/**
 * Classe responsÃ¡vel pela validaÃ§ao SintÃ¡tica do documento NCL.
 * @author roberto
 * 
 */
public class DTDValidator {
	...
	public static boolean validate(String fileName, Element root) {
		...
		if(!nclStructure.isElement(tagName)) {  
			...
			MessageList.addError(fileName, "Elemento desconhecido '" + tagName + "'. Este programa não checa estes tipo de elementos filho.", root, MessageList.PORTUGUESE);			
			return false;
		}
		...
		for(int i=0; i<attList.getLength(); i++) {
			...
			if( node.getNodeType() == Node.ATTRIBUTE_NODE) {		
				if(!nclStructure.isAttribute(tagName, node.getNodeName())) {
					...
					MessageList.addError(fileName, "Atributo inválido '" + strName + "' no elemento '" + tagName + "'.", root, MessageList.PORTUGUESE);
					ret = false;
				}
		...
		while(it.hasNext()){
			...
			if(entry.getValue().booleanValue() && !attSet.contains(entry.getKey())){
				...
				MessageList.addError(fileName, "O atributo '" + entry.getKey() + "' é obrigatório mas não está presente no elemento <" + tagName + ">.", root, MessageList.PORTUGUESE);
				ret = false;
			}
		}
		...
		for(int i=0; i<attList.getLength(); i++) {
			...
			if( node.getNodeType() == Node.ATTRIBUTE_NODE) {
				...
				if(!DataType.isDataType(mdt.intValue(), value)) {
					...
					MessageList.addError(fileName, "Tipo de dado inválido para o atributo '" + strName + "' do elemento <" + tagName + ">. Identificador = "+root.getAttribute("id")+".", root, MessageList.PORTUGUESE);
					ret = false;
				}
			}
		}
		...
		for(int i=0; i<childList.getLength(); i++) {
			...
			if( node.getNodeType() == Node.ELEMENT_NODE ) {
				...
				if( nclStructure.isChild(tagName, childTagName) ) {
					...
					MessageList.addError(fileName, "O elemento <" + childTagName + "> não é um filho válido do elemento <'" + tagName + ">.", child, MessageList.PORTUGUESE);
					ret = false;
				}
			}
		}
			...
			switch(ch) {
				case '?':
					if( childSet.containsKey(entry.getKey()) && childSet.get(entry.getKey())>1 ) {
						...
						MessageList.addError(fileName, "O elemento <" + tagName + "> possui mas de um filho <" + entry.getKey() + "> cuja cardinalidade é opcional (0 ou 1 ocorrências).", root, MessageList.PORTUGUESE);
						ret = false; 
					}
				break;
				case '+':
