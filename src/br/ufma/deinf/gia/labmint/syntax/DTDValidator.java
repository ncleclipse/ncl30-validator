...
public class DTDValidator {
	...
	public static boolean validate(String fileName, Element root) {
		...
		if(!nclStructure.isElement(tagName)) {  
			MessageList.addError(fileName, "Unknown element '" + tagName + "'. This program does not check its child elements.", root); 
			return false;
		}
		...
		for(int i=0; i<attList.getLength(); i++) {
			...
			if( node.getNodeType() == Node.ATTRIBUTE_NODE) {		
				if(!nclStructure.isAttribute(tagName, node.getNodeName())) {
					...
					MessageList.addError(fileName, "Invalid attribute '" + strName + "' at '" + tagName + "' element.", root);
					ret = false;
				}
		...
		while(it.hasNext()){
			...
			if(entry.getValue().booleanValue() && !attSet.contains(entry.getKey())){
				MessageList.addError(fileName, "The attribute '" + entry.getKey() + "' is mandatory but is not present at '" + tagName + "' element.", root);
				ret = false;
			}
		}
		...
		for(int i=0; i<attList.getLength(); i++) {
			...
			if( node.getNodeType() == Node.ATTRIBUTE_NODE) {
				...
				if(!DataType.isDataType(mdt.intValue(), value)) {
					String strName = node.getNodeName();
					MessageList.addError(fileName, "Invalid data type of attribute '" + strName + "' at '" + tagName + "' element. Id = "+root.getAttribute("id")+".", root);
					ret = false;
				}
			}
		}
		...
		for(int i=0; i<childList.getLength(); i++) {
			...
			if( node.getNodeType() == Node.ELEMENT_NODE ) {
				...
				if( nclStructure.isChild(tagName, childTagName) ) {
					...
					MessageList.addError(fileName, "The element '" + childTagName + "' is an invalid child of '" + tagName + "' element.", child);
					ret = false;
				}
			}
		}
			...
			switch(ch) {
				case '?':
					if( childSet.containsKey(entry.getKey()) && childSet.get(entry.getKey())>1 ) {
						MessageList.addError(fileName, "The <" + tagName + "> element has more than one <" + entry.getKey() + "> child element that is optional (0 or 1 occurence).", root);
						ret = false; 
					}
				break;
				case '+':
					if( !childSet.containsKey(entry.getKey()) ) {
						MessageList.addError(fileName, "The <" + tagName + "> element must have at least one <" + entry.getKey() + "> child element.", root);
						ret = false;
					}
				break;
				case '1':
					if( !childSet.containsKey(entry.getKey()) || childSet.get(entry.getKey())!=1 ) {
						MessageList.addError(fileName, "The <" + tagName + "> element must have exactly one <" + entry.getKey() + "> child element.", root);
						ret = false;
					}					
				break;
				case '#':
		...
		if(checkAtLeastOne && !flagAtLeastOne) {
			MessageList.addError(fileName, "The element <" + tagName + "> must have at least one child element (cardinality #).", root);
			ret = false;
		}

		if(checkAtLeastA && !flagAtLeastA) {
			MessageList.addError(fileName, "The element <" + tagName + "> must have one child element (cardinality a).", root);
			ret = false;
		}

		if(checkAtLeastB && !flagAtLeastB) {
			MessageList.addError(fileName, "The element <" + tagName + "> must have one child element (cardinality b).", root);
			ret = false;
		}
		...
		if(checkAssessmentStatement) {
			...
			if(!((attribute==2&&value==0) || (attribute==1&&value==1) ) ) { 
				MessageList.addError(fileName, "The element <assessmentStatement> must have two children <attributeAssessment> elements or"
						+ " an <attributeAssessment> and a <valueAssessment> children elements.", root);
				ret = false;
			}
		}
